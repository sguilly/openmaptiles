...
WHERE
    length(ref)>0 AND
    ST_Intersects(geometry, (SELECT * FROM ne_10m_admin_0_bg_buffer)) AND
...
SELECT * FROM gbr_route_members_view;
...
WHERE network != '' AND network_type != osm_route_member_network_type(network, name, ref)
;
...
BEGIN
    ...
    WHERE
        network IN ('omt-gb-motorway', 'omt-gb-trunk') AND
        r.osm_id = c.osm_id
    ;

    INSERT INTO
        osm_route_member (osm_id, member, ref, network)
    SELECT
        r.*
    FROM
        gbr_route_members_view AS r
    ...
    UPDATE
        osm_route_member AS r
    SET
        network_type = osm_route_member_network_type(network, name, ref)
    FROM
        transportation_name.network_changes AS c
    WHERE
        network != '' AND
        network_type != osm_route_member_network_type(network, name, ref) AND
        r.member = c.osm_id
    ;
END;
$$ LANGUAGE plpgsql;
